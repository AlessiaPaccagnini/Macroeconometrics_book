...
% simulated the USA as a VAR(2) but stacked only the lag-1 block, which
% inflated interest-rate persistence and pushed the global system past the
% unit circle; a VAR(1) throughout keeps the stack consistent and stable.)
% Impulse responses are in the variables' own decimal units, matching the
% small magnitudes on the book's Figure 13.3 axes (x10^-3 / x10^-4).
% -------------------------------------------------------------------------
%
...
% trade-scaled to US exposure (Canada strongest, wCAN,USA=0.55). Well identified,
% so OLS recovers the loadings and the global eigenvalue/FEVD match the book.
% Matches the Python and R scripts.
A1_US = [ 0.50,  0.00, -0.181;
          0.09,  0.55,  0.06;
          0.22,  0.12,  0.80];
GAMMA_OIL_US = [-0.06; 0.08; 0.02];

% Phi_i (diagonal persistence) for non-US countries
PHI  = struct();
LAM0 = struct();

PHI.Euro_Area  = diag([0.50, 0.62, 0.8151]);
PHI.UK         = diag([0.48, 0.58, 0.7952]);
PHI.Japan      = diag([0.44, 0.52, 0.8449]);
PHI.China      = diag([0.60, 0.65, 0.8350]);
PHI.Canada     = diag([0.52, 0.58, 0.8051]);
PHI.Korea      = diag([0.46, 0.54, 0.7753]);
PHI.Brazil     = diag([0.40, 0.68, 0.7356]);

% Lambda0: dominant entry gdp<-gdp* (foreign-GDP gain, trade-scaled to US)
gdp_gain = struct('Euro_Area',0.20,'UK',0.16,'Japan',0.14,'China',0.10, ...
...
ALPHA.Brazil   = [0.004; 0.006; 0.070];
