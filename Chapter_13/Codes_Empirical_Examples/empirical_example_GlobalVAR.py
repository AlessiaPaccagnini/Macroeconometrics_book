...
--------------------------------------------------------------------------------
The Python, MATLAB, and R companion scripts are designed to produce IDENTICAL
numbers. To guarantee this, the dataset is generated ONCE by this Python script
and written to a shared CSV file (gvar_simulated_data.csv). The MATLAB and R
scripts then READ that same CSV instead of re-simulating, so estimation, the
global stacking, the GIRF, the FEVD, and the Table 13.7 weak-exogeneity tests
are run on exactly the same data in all three languages.

If the CSV is absent, this Python script regenerates it (seed 42). The MATLAB
and R scripts expect the CSV to be present in the working directory; run the
Python script first (or copy the CSV alongside them).

Model (book eq. 13.74): each non-US country is a VARX*(1,1); the USA is the
dominant unit and is a closed VAR(1) with weakly exogenous oil. (Earlier drafts
simulated the USA as a VAR(2) but stacked only the lag-1 block, which inflated
interest-rate persistence and pushed the global system past the unit circle;
using a VAR(1) throughout keeps the stack internally consistent and the system
stable.) Impulse responses and FEVD-related GDP responses are reported in the
variables' own (decimal) units — matching the small magnitudes on the book's
Figure 13.3 axes (×10⁻³/×10⁻⁴), not rescaled to percentage points.
--------------------------------------------------------------------------------
================================================================================
...
DGP calibrated to reproduce the book's published figures/tables:
  Table 13.7:  F-stats < 3.07 across the panel (weak exogeneity holds), with
               the Brazil rs* cell as the documented high-volatility outlier
  Figure 13.2: Max eigenvalue modulus ≈ 0.928 (dynamically stable)
  Figure 13.3: GIRF GDP responses to the US rate shock are SMALL (note the
               ×10⁻³/×10⁻⁴ axes). The US troughs at ≈ −1.2×10⁻³ at h=2.
               Because the GIRF conditions on the historical covariance, several
               partners (e.g. Canada, Japan) move POSITIVE on impact before the
               spillover turns them negative — the GIRF≠structural attenuation
               the chapter discusses explicitly.
  Figure 13.4: Own shocks 88–97% of GDP FEV (h=8);
               USA → Canada ≈ 8% (largest spillover), USA → others ≤ ~5%,
               foreign → USA ≈ 6% (dominant-unit asymmetry preserved).
  (Responses are reported in the variables' own decimal units, NOT ×100.)

Script steps:
  1.  Generate GVAR data and write the shared CSV (gvar_simulated_data.csv)
  2.  Construct trade-weighted foreign variables  (eq. 13.67)
  3.  Estimate VARX*(1,1) for non-US; closed VAR(1)+oil for USA
...
# USA = dominant unit, closed VAR(1) + weakly exogenous oil.
# Interest-rate persistence (rs own-coeff 0.80) plus the foreign-GDP spillover
# channel below produce the near-unit-root global eigenvalue (~0.93) of Fig. 13.2.
# A1_US[0,2] (rate -> GDP) drives the US GIRF GDP trough (~-1.2e-3 at h=2).
A1_US = np.array([
    [ 0.50,  0.00, -0.181],   # GDP:  own persistence + rate drag (calibrated)
...
# Transmission runs through the trade-weighted FOREIGN GDP channel
# (Lambda0[gdp, gdp] > 0): a foreign-activity downturn pulls domestic GDP down.
# This channel is well identified (foreign GDP is far less collinear than the
# near-unit-root foreign rate), so OLS recovers the calibrated loadings and the
# global FEVD/eigenvalue reproduce the book. The gdp<-gdp gain scales with trade
# exposure to the US, so Canada (wCAN,USA=0.55) has by far the strongest link.
# PHI_i: diagonal persistence matrices (matching MATLAB/R specification)
PHI = {
    "Euro_Area": np.diag([0.50, 0.62, 0.8151]),
    "UK":        np.diag([0.48, 0.58, 0.7952]),
    "Japan":     np.diag([0.44, 0.52, 0.8449]),
    "China":     np.diag([0.60, 0.65, 0.8350]),
    "Canada":    np.diag([0.52, 0.58, 0.8051]),
    "Korea":     np.diag([0.46, 0.54, 0.7753]),
    "Brazil":    np.diag([0.40, 0.68, 0.7356]),
}
# Lambda_0: contemporaneous foreign effect. Dominant entry is gdp<-gdp* (row 0,
# col 0): the trade-weighted foreign-GDP gain. Canada largest (0.40), reflecting
# wCAN,USA=0.55; smaller, trade-scaled gains for the others. The mild
# inf<-*, rs<-* entries keep the inflation/rate blocks realistic.
_GDP_GAIN = {
    "Euro_Area": 0.20, "UK": 0.16, "Japan": 0.14, "China": 0.10,
    "Canada": 0.40, "Korea": 0.16, "Brazil": 0.14,
}
LAM0 = {
    c: np.array([[_GDP_GAIN[c], 0.04, 0.00],
                 [0.04,         0.08, 0.04],
                 [0.06,         0.04, 0.14]])
    for c in _GDP_GAIN
}
# Contemporaneous-only foreign transmission (no lagged-foreign term) keeps the
# spillover propagating with minimal delay and avoids the rs* multicollinearity
# that previously corrupted the estimated loadings.
LAM1 = {c: np.zeros((K, K)) for c in LAM0}
...
SIG = {
    ...
    "Brazil":   np.array([0.008, 0.004, 0.024]),   # low GDP/inf noise, high rate noise
}
ALPHA = {
    ...
    "Brazil":   np.array([0.004, 0.006, 0.070]),   # elevated rate intercept -> rs* outlier
}
...
def simulate_gvar(seed: int = 42) -> dict:
    ...
    for t in range(2, T):
        # ── USA (dominant unit, closed VAR(1) + oil) ─────────────────────────
...
# ── shared CSV: generate once in Python, read in MATLAB and R ─────────────────
CSV_PATH = "gvar_simulated_data.csv"

def save_csv(country_data: dict, path: str = CSV_PATH):
    """Write the simulated panel to a long-format CSV so MATLAB/R read identical data.
    Columns: date, country, gdp, inf, rs
    """
    frames = []
    for c in COUNTRIES:
        df = country_data[c].copy()
        df.insert(0, "country", c)
        df.insert(0, "date", df.index.astype(str))
        frames.append(df.reset_index(drop=True))
    out = pd.concat(frames, ignore_index=True)
    out.to_csv(path, index=False)
    print(f"    Shared dataset written → {path}")

def load_csv(path: str = CSV_PATH) -> dict:
    """Read the shared CSV back into the country → DataFrame dict."""
    raw = pd.read_csv(path)
    data = {}
    for c in COUNTRIES:
        sub = raw[raw["country"] == c].reset_index(drop=True)
        data[c] = pd.DataFrame(sub[VARS].values, columns=VARS,
                               index=pd.to_datetime(sub["date"]))
    return data

def get_data(seed: int = 42, csv_path: str = CSV_PATH) -> dict:
    """Load the shared CSV if present; otherwise simulate and write it."""
    import os
    if os.path.exists(csv_path):
        print(f"    Loading shared dataset ← {csv_path}")
        return load_csv(csv_path)
    print("    No shared CSV found — simulating and writing it …")
    cd = simulate_gvar(seed)
    save_csv(cd, csv_path)
    return cd
...
def estimate_var_usa(Y: np.ndarray, p: int = 1) -> dict:
    """
    Pure VAR(p) for USA — no foreign variables (dominant unit assumption).
    USA is a closed economy: Lam0 = Lam1 = 0. Default p=1 so the global
    1-lag stack is internally consistent (see header note).
...
def estimate_all(country_data: dict, foreign: dict) -> dict:
    ...
    USA uses a dedicated pure VAR(1) — no foreign variables (dominant unit).
    ...
    for c in COUNTRIES:
        ...
        if c == "USA":
            models[c] = estimate_var_usa(Y, p=1)
        else:
...
def print_table_13_7(df: pd.DataFrame):
    ...
    print("="*52)
    print("\nMost F-stats fall well below 3.07; weak exogeneity holds")
    print("  across the panel, with only the UK Δy* and Korea rs* cells")
    print("  marginally above the critical value.")
...
def plot_fig13_2(F: np.ndarray, path: str = None) -> float:
    ...
    # No internal title: the book caption carries the figure numbering.
    ...
    if path:
        ...
        plt.savefig(path.rsplit(".", 1)[0] + ".png", dpi=300, bbox_inches="tight")
...
def compute_girf(F: np.ndarray, Sigma_e: np.ndarray,
    ...
    Responses are in the same (decimal) units as the variables, matching the
    magnitudes plotted in the book's Figure 13.3 (note the ×10⁻³/×10⁻⁴ axes).
...
def plot_fig13_3(irf: dict, irf_lo: dict, irf_hi: dict,
    ...
    for idx, c in enumerate(COUNTRIES):
        ...
        # Peak annotation (trough), offset scaled to this panel's range
        th = int(np.argmin(gdp))
        span = max(hi_g.max() - lo_g.min(), 1e-9)
        ax.annotate(f"{gdp[th]:.1e}",
                    xy=(th, gdp[th]),
                    xytext=(th + 2.0, gdp[th] - 0.18 * span),
                    fontsize=7, color="#C00000", ha="left",
                    arrowprops=dict(arrowstyle="->", color="#C00000", lw=0.7))
    ...
    # No suptitle / figure number inside the image: the caption goes in LaTeX.
    plt.tight_layout()
    if path:
        plt.savefig(path, bbox_inches="tight")
        plt.savefig(path.rsplit(".", 1)[0] + ".png", dpi=300, bbox_inches="tight")
...
def main():
    ...
    print("\n[1] Generating GVAR data  (T=162, N=8) …")
    # Generate fresh and (over)write the shared CSV so MATLAB and R read identical data.
    country_data = simulate_gvar(seed=42)
    save_csv(country_data, CSV_PATH)
    print(f"    Countries: {COUNTRIES}")
    print(f"    wCAN,USA = {W[CIDX['Canada'], CIDX['USA']]:.2f}")
    ...
    print(sep)
    print(f"  Max eigenvalue:      {max_eig:.3f}")
    print()
    print("  Trough GDP response to 1 s.d. US rate shock (Figure 13.3, decimal units):")
    for c in COUNTRIES:
        gdp = irf[c][:, 0]
        th  = int(np.argmin(gdp))
        print(f"    {c.replace('_',' '):<12}: {gdp[th]:+.2e} at h={th}")
    print()
    print("  GDP FEVD at h=8 (Figure 13.4) — diagonal = own shocks:")
    own = np.diag(fevd)
    print(f"    Own shocks range: {own.min()*100:.0f}–{own.max()*100:.0f}%")
    can_i = CIDX["Canada"]; usa_i = CIDX["USA"]
    print(f"    USA → Canada:  {fevd[can_i, usa_i]*100:.0f}%")
    eur_i = CIDX["Euro_Area"]; uk_i = CIDX["UK"]
    print(f"    EUR → UK:      {fevd[uk_i, eur_i]*100:.0f}%")
    print(f"    Canada → USA:  {fevd[usa_i, can_i]*100:.0f}%")
    print(sep)
    print(f"\n  Figures saved:")
