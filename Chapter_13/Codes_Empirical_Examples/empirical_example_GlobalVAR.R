...
# numbers. The dataset is generated ONCE by the Python script and written to a
# shared CSV (gvar_simulated_data.csv). THIS R script READS that CSV rather than
# simulating, so estimation, the global stacking, the GIRF, the FEVD, and the
# Table 13.7 weak-exogeneity tests run on exactly the same data in all three
# languages.
#
...
# dominant unit and a closed VAR(1) with weakly exogenous oil. (Earlier drafts
# simulated the USA as a VAR(2) but stacked only the lag-1 block, which inflated
# interest-rate persistence and pushed the global system past the unit circle; a
# VAR(1) throughout keeps the stack consistent and the system stable.) Impulse
# responses are in the variables' own decimal units, matching the small
# magnitudes on the book's Figure 13.3 axes (x10^-3 / x10^-4).
# -----------------------------------------------------------------------------
#
# Book specification (eq. 13.74):
#   y_it = alpha_i + Phi_i y_{i,t-1} + Lam0_i y*_it
#                  + Lam1_i y*_{i,t-1} + eps_it
#   USA: closed VAR(1) + oil (dominant unit)
...
# ── DGP parameters ───────────────────────────────────────────────────────────
# Transmission via the trade-weighted FOREIGN GDP channel (LAM0[gdp,gdp] > 0),
# scaled by trade exposure to the US (Canada strongest, wCAN,USA=0.55). This
# channel is well identified, so OLS recovers the loadings and the global
# eigenvalue/FEVD reproduce the book. Matches the Python and MATLAB scripts.
A1_US <- matrix(c( 0.50,  0.00, -0.181,
                   0.09,  0.55,  0.06,
                   0.22,  0.12,  0.80), 3, 3, byrow=TRUE)
GAMMA_OIL_US <- c(-0.06, 0.08, 0.02)

# Phi (diagonal persistence) for non-US countries
PHI  <- list(
  Euro_Area = diag(c(0.50, 0.62, 0.8151)),
  UK        = diag(c(0.48, 0.58, 0.7952)),
  Japan     = diag(c(0.44, 0.52, 0.8449)),
  China     = diag(c(0.60, 0.65, 0.8350)),
  Canada    = diag(c(0.52, 0.58, 0.8051)),
  Korea     = diag(c(0.46, 0.54, 0.7753)),
  Brazil    = diag(c(0.40, 0.68, 0.7356))
)
# Lambda0: dominant entry gdp<-gdp* (foreign-GDP gain, trade-scaled to US).
GDP_GAIN <- list(Euro_Area=0.20, UK=0.16, Japan=0.14, China=0.10,
                 Canada=0.40, Korea=0.16, Brazil=0.14)
LAM0 <- lapply(names(GDP_GAIN), function(c) matrix(c(
            GDP_GAIN[[c]], 0.04, 0.00,
            0.04,          0.08, 0.04,
            0.06,          0.04, 0.14), 3, 3, byrow=TRUE))
names(LAM0) <- names(GDP_GAIN)
# Contemporaneous-only foreign transmission (no lagged-foreign term).
LAM1 <- lapply(LAM0, function(x) 0 * x)
...
SIG <- list(
  ...
  Korea=c(0.012,0.005,0.008),Brazil=c(0.008,0.004,0.024)
)
ALPHA <- list(
  ...
  Korea=c(0.008,0.004,0.060),Brazil=c(0.004,0.006,0.070)
)
...
# =============================================================================
# FIGURES
# (No internal titles or book figure numbers: the LaTeX captions carry the
#  numbering. Each figure is saved as PDF and 300-dpi PNG.)
# =============================================================================
save_both <- function(path_pdf, width, height, draw_fun) {
...
plot_fig13_2 <- function(F_mat, path=NULL) {
  ...
  draw <- function() {
    ...
    text(1.28, -1.28, bquote("Max |" * lambda * "| = " * .(sprintf("%.3f", max_mod))),
         col="#C00000", cex=0.9, adj=c(1,0))
  }
  if (is.null(path)) draw() else save_both(path, 6, 6, draw)
  max_mod
}

plot_fig13_3 <- function(irf, irf_lo, irf_hi, H=20, path=NULL) {
  draw <- function() {
    par(mfrow=c(2,4), mar=c(3.2,4.6,2,1), oma=c(0,0,0,0), mgp=c(3.3,0.6,0))
    h_ax <- 0:(H-1)
    col_ <- "#2E75B6"
    for (i in seq_len(N)) {
      c   <- COUNTRIES[i]
      ...
      plot(h_ax, gdp, type="n", xlab="", ylab="", main=gsub("_"," ",c),
           ylim=ylim_, cex.main=0.9, las=1, cex.axis=0.75); grid(col="grey90")
      title(xlab="Quarters", line=2.0, cex.lab=0.85)
      title(ylab="GDP growth (pp)", line=3.5, cex.lab=0.85)
      polygon(c(h_ax,rev(h_ax)), c(lo_,rev(hi_)), col=adjustcolor(col_,0.20), border=NA)
      lines(h_ax, gdp, col=col_, lwd=2.0)
      abline(h=0, lwd=0.6)
      th <- which.min(gdp)
      adj_x <- if ((th-1) > 0.7*(H-1)) c(1, 1.3) else c(0, 1.3)  # keep label inside
      text(th-1, gdp[th], sprintf("%.1e", gdp[th]),   # x in h units (0-based)
           col="#C00000", cex=0.7, adj=adj_x)
    }
  }
  if (is.null(path)) draw() else save_both(path, 14, 7, draw)
}
...
plot_fig13_4 <- function(fevd, path=NULL) {
  ...
  draw <- function() {
    ...
    image(1:N, 1:N, t(pct[N:1,]),  # flip for conventional row/col display
          ...
          xaxt="n", yaxt="n",
          xlab="Shock source (country j)", ylab="", main="")
    ...
    plot(0, 0, type="n", xlim=c(-1.4,1.4), ylim=c(-1.4,1.4),
         asp=1, xlab="", ylab="", main="", axes=FALSE)
  ...
  if (is.null(path)) draw() else save_both(path, 14, 5.5, draw)
}
...
cat("\n[1] Loading shared GVAR data <- gvar_simulated_data.csv ...\n")
load_csv <- function(path = "gvar_simulated_data.csv") {
  if (!file.exists(path))
    stop(sprintf(paste0("Shared dataset not found: %s\n",
         "Run empirical_example_GlobalVAR.py first (it writes the CSV), ",
         "or copy the CSV into this directory."), path))
  raw <- read.csv(path, stringsAsFactors = FALSE)
  setNames(lapply(COUNTRIES, function(c) {
    sub <- raw[raw$country == c, c("gdp", "inf", "rs")]
    as.matrix(sub)
  }), COUNTRIES)
}
Y_data <- load_csv()
cat(sprintf("    wCAN,USA = %.2f\n", W[CIDX["Canada"], CIDX["USA"]]))

cat("\n[2] Constructing foreign variables ...\n")
foreign <- make_foreign(Y_data)

estimate_var_usa <- function(Y, p = 1) {
  # Pure VAR(p) for USA — no foreign variables (dominant unit assumption).
  # Default p=1 so the global 1-lag stack is internally consistent.
...
models <- lapply(setNames(COUNTRIES, COUNTRIES), function(c) {
  Y <- Y_data[[c]]
  m <- if (c == "USA") estimate_var_usa(Y, p=1L)
...
cat("RESULTS SUMMARY\n")
cat(strrep("=",70), "\n")
cat(sprintf("  Max eigenvalue: %.3f\n", max_eig))
cat("\n  Trough GDP response to 1 s.d. US rate shock (decimal units):\n")
for (c in COUNTRIES) {
  gdp <- irf[[c]][,1]
  th  <- which.min(gdp)
  cat(sprintf("    %-12s: %+.2e at h=%d\n",
              gsub("_"," ",c), gdp[th], th-1))
}
own <- diag(pct)
cat(sprintf("\n  Own shocks: %d-%d%%\n", min(own), max(own)))
cat(sprintf("  USA -> Canada: %d%%\n",   pct[can_i, usa_i]))
cat(sprintf("  EUR -> UK:     %d%%\n",   pct[uk_i,  eur_i]))
cat(sprintf("  Canada -> USA: %d%%\n",   pct[usa_i, can_i]))
cat(strrep("=",70), "\n")
