...
# =============================================================================
#
# NOTE: This R port reproduces the METHODOLOGY and qualitative findings of
# Section 13.7.1. Exact magnitudes differ from the printed Table 13.6 because
# random-number generators differ across languages; the canonical table/figure
# values are produced by the Python companion script. Confidence bands use a
# sieve bootstrap (Appendix 13A).
#
#
...
bootstrap_irf <- function(country_data, countries, H = 20, n_boot = 1000,
                           shock_col = 3, seed = 1, p = 2) {
  # Bias-corrected SIEVE bootstrap for IRF confidence bands.
  #
  # Resamples the estimated residuals and regenerates each country's
  # series THROUGH the estimated dynamics (Appendix 13A; Dees et al. 2007),
  # preserving serial dependence. Naive row-resampling of observations
  # would destroy the lag structure and collapse the bands.
  #
  # The plotted line is the POINT estimate (matches Table 13.6 / text);
  # the bootstrap is used only for the 90% bands.
  set.seed(seed)
  k <- 3L

  # Point estimate on observed panel
  res_pt <- build_panel(country_data, countries, p)
  P_pt   <- t(chol(res_pt$Sig))
  irf_pt <- irf_from_coeffs(res_pt$B, P_pt, shock_col, H, k)
  B_pt   <- res_pt$B

  # Collect within-equation residuals (GDP & inflation) and metadata
  resid_pool <- list(); meta <- list()
  for (c_name in countries) {
    Y  <- country_data[[c_name]]
    Tc <- nrow(Y); Te <- Tc - p
    Yd <- Y[(p+1):Tc, ]
    Xl <- do.call(cbind, lapply(seq_len(p), function(lag) Y[(p-lag+1):(Tc-lag), ]))
    U  <- sweep(Yd, 2, colMeans(Yd)) - sweep(Xl, 2, colMeans(Xl)) %*% B_pt
    resid_pool[[c_name]] <- U[, 1:2, drop = FALSE]   # GDP & inflation innovations
    meta[[c_name]] <- list(Y = Y, Tc = Tc)
  }
  Rt <- do.call(rbind, resid_pool)

  # Sieve step: short VAR(p_boot) on residual innovations
  p_boot <- max(1L, min(4L, as.integer(round(nrow(Rt)^(1/3)) %/% 10L)))
  if (p_boot < 1L) p_boot <- 1L
  Zlag <- do.call(cbind, lapply(seq_len(p_boot),
            function(j) Rt[(p_boot - j + 1):(nrow(Rt) - j), , drop = FALSE]))
  Rdep <- Rt[(p_boot + 1):nrow(Rt), , drop = FALSE]
  if (nrow(Rdep) > ncol(Zlag)) {
    Psi      <- solve(crossprod(Zlag), crossprod(Zlag, Rdep))   # (2*p_boot x 2)
    whitened <- Rdep - Zlag %*% Psi
  } else {
    Psi <- NULL; whitened <- Rt
  }
  whitened <- sweep(whitened, 2, colMeans(whitened))
  ...
  for (b in seq_len(n_boot)) {
    ...
    for (c_name in countries) {
      ...
      innov <- whitened[sample.int(nrow(whitened), Tc, replace = TRUE), , drop = FALSE]
      if (!is.null(Psi)) {
        e <- innov
        for (t in (p_boot + 1):Tc) {
          zl <- as.numeric(t(sapply(seq_len(p_boot), function(j) e[t - j, ])))
          e[t, ] <- innov[t, ] + zl %*% Psi
        }
      } else e <- innov

      for (t in (p + 1):Tc) {
        xlag <- as.numeric(t(sapply(seq_len(p), function(lag) Yb[t - lag, ])))
        yhat <- xlag %*% B_pt
        Yb[t, 1] <- yhat[1] + e[t, 1]
        Yb[t, 2] <- yhat[2] + e[t, 2]
      }
      colnames(Yb) <- c("gdp", "inf", "oil")
      boot[[c_name]] <- Yb
    }
    tryCatch({
      res_b  <- build_panel(boot, countries, p)
      P_b    <- t(chol(res_b$Sig))
      cand   <- irf_from_coeffs(res_b$B, P_b, shock_col, H, k)
      if (any(!is.finite(cand))) boots[b,,] <- irf_pt else boots[b,,] <- cand
    }, error = function(e) { boots[b,,] <<- irf_pt })
  }

  # Recentered (pivotal) bands: with very small groups (e.g. N_exp = 4) the
  # raw percentile band can drift off the point estimate. Centering the
  # bootstrap DISPERSION on irf_pt yields a band that brackets the point
  # estimate while still reflecting sampling uncertainty.
  irf_mean <- apply(boots, c(2,3), mean)
  dev_lo   <- apply(boots, c(2,3), quantile, 0.05) - irf_mean
  dev_hi   <- apply(boots, c(2,3), quantile, 0.95) - irf_mean
  lo <- irf_pt + dev_lo
  hi <- irf_pt + dev_hi
  # Plot the POINT estimate; bootstrap only informs the bands.
  list(irf = irf_pt, lo = lo, hi = hi)
}
...
plot_fig13_1 <- function(irf_imp, lo_imp, hi_imp,
  ...
  for (v in 1:2) {
    ...
    ylims <- sort(ylims + c(-1,1)*0.1*diff(ylims))   # ascending: neg down, pos up
...
print_table_13_6 <- function(B_pool, t_pool, B_imp, t_imp, B_exp, t_exp, W, pval) {
  ...
  cat("Published Table 13.6: Pooled=-0.012(t=-2.03)  Imp=-0.024(t=-6.21)  Exp=+0.014(t=+1.79)\n")
  cat(strrep("=",78), "\n\n")
  cat("Published Table 13.6: Wald chi2(18)=192.4, p<0.001\n")
}
...
cat(strrep("=",70), "\n")
cat(sprintf("  Pooled  Dpoil->GDP:  %+.3f (t=%+.2f)  [book: -0.012, t=-2.03]\n",
            res_pool$B[3,1], t_pool[3]))
cat(sprintf("  Importer Dpoil->GDP: %+.3f (t=%+.2f)  [book: -0.024, t=-6.21]\n",
            res_imp$B[3,1], t_imp[3]))
cat(sprintf("  Exporter Dpoil->GDP: %+.3f (t=%+.2f)  [book: +0.014, t=+1.79]\n",
            res_exp$B[3,1], t_exp[3]))
cat(sprintf("  Wald chi2(18) = %.1f (p=%.3f)  [book: 192.4, p<0.001]\n", wt$W, wt$pval))
