...
% GDP growth and inflation are annualised (x400) -> annualised percentage
% points; the federal funds rate is in percentage points.
y_units = {'Annualised pp','Annualised pp','Percentage points'};
fig2 = figure('Name','TVP-VAR-SV IRFs','Position',[50 50 1100 380]);
hor  = 0:HORZ-1;
for i = 1:N
    subplot(1,N,i);
    % Normal times (blue)
    hbn = fill([hor, fliplr(hor)], [irf_n_lo(:,i)', fliplr(irf_n_hi(:,i)')], ...
        [0.13 0.40 0.67], 'FaceAlpha',0.20,'EdgeColor','none', ...
        'DisplayName','68% credible interval (normal)'); hold on;
    hln = plot(hor, irf_n_med(:,i), 'Color',[0.13 0.40 0.67], 'LineWidth',2.0, ...
        'DisplayName','Posterior mean, normal times (2005Q1)');
    % Stress times (red dashed)
    hbs = fill([hor, fliplr(hor)], [irf_s_lo(:,i)', fliplr(irf_s_hi(:,i)')], ...
        [0.84 0.37 0.30], 'FaceAlpha',0.20,'EdgeColor','none', ...
        'DisplayName','68% credible interval (stress)');
    hls = plot(hor, irf_s_med(:,i), '--','Color',[0.84 0.37 0.30], 'LineWidth',2.0, ...
        'DisplayName','Posterior mean, stress (2008Q4)');
    yline(0,'k:','LineWidth',0.8);
    xlabel('Quarters ahead'); ylabel(y_units{i}); title(resp_labels{i});
    if i==1
        legend([hbn hln hbs hls],'Location','southwest','FontSize',7);
    end
    grid on; grid minor;
end
sgtitle({'TVP-VAR-SV: Response to a 1 SD Contractionary Monetary Policy Shock', ...
    'Normal Times (2005Q1) vs. Financial-Crisis Stress (2008Q4)'}, ...
