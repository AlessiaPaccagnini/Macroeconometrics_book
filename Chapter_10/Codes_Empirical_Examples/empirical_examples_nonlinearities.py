...
# Sample 1960Q1-2019Q4 (240 obs); model index 0 = first obs AFTER dropping
# LAGS rows, i.e. 1960Q3. So model index = (quarters since 1960Q1) - LAGS.
# 2005Q1 -> 181 - 1 - LAGS = 178 ; 2008Q4 -> 196 - 1 - LAGS = 193  (LAGS=2)
# 2005Q1 = model index 178  (0-based)
# 2008Q4 = model index 193  (0-based)
T_NORMAL = 178
T_STRESS = 193
...
hor = np.arange(HORZ)
# GDP growth and inflation are annualised (x400) -> annualised percentage points;
# the federal funds rate is in percentage points.
y_units = ["Annualised pp", "Annualised pp", "Percentage points"]
fig, axes = plt.subplots(1, 3, figsize=(13,4))
fig.suptitle("TVP-VAR-SV: Response to a 1 SD Contractionary Monetary Policy Shock\n"
...
for i, ax in enumerate(axes):
    ax.fill_between(hor, irf_n_lo[:,i], irf_n_hi[:,i], alpha=0.2, color="#2166ac",
                    label="68% credible interval (normal)")
    ax.plot(hor, irf_n_med[:,i], color="#2166ac", lw=2,
            label="Posterior mean, normal times (2005Q1)")
    ax.fill_between(hor, irf_s_lo[:,i], irf_s_hi[:,i], alpha=0.2, color="#d6604d",
                    label="68% credible interval (stress)")
    ax.plot(hor, irf_s_med[:,i], color="#d6604d", lw=2, ls="--",
            label="Posterior mean, stress (2008Q4)")
    ax.axhline(0, color="k", lw=0.6, ls=":"); ax.grid(True, alpha=0.25, ls=":")
    ax.set_xlabel("Quarters ahead"); ax.set_ylabel(y_units[i])
    ax.set_title(resp_labels[i])
