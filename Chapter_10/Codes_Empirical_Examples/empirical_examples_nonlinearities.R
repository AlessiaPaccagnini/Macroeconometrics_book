...
# Sample 1960Q1-2019Q4 (240 obs); the IRF grid is indexed AFTER dropping
# LAGS rows (grid element 1 = 1960Q3, 1-based).
# 2005Q1 -> 181 - LAGS = 179 ; 2008Q4 -> 196 - LAGS = 194  (LAGS = 2)
T_NORMAL  <- 179L
T_STRESS  <- 194L
...
save_fig("fig_tvpvar_irf_comparison.png", width=13, height=4, {
  ...
  for (i in seq_len(N)) {
    ...
    if (i==1) legend("bottomleft",
      legend=c("Posterior mean, normal times (2005Q1)",
               "68% credible interval (normal)",
               "Posterior mean, stress (2008Q4)",
               "68% credible interval (stress)"),
      col=c("#2166ac", adjustcolor("#2166ac",0.20),
            "#d6604d", adjustcolor("#d6604d",0.20)),
      lty=c(1,NA,2,NA), lwd=c(2,NA,2,NA),
      pch=c(NA,15,NA,15), pt.cex=1.6, bty="n", cex=0.7)
  }
  mtext(paste0("TVP-VAR-SV: Response to a 1 SD Contractionary Monetary Policy Shock\n",
