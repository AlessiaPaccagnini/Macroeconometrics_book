...
# Helper: smooth bootstrap median with moving average, clipped inside bands
smooth_median <- function(med, lo, hi, window = 5) {
  n <- length(med)
  med_s <- stats::filter(med, rep(1/window, window), sides = 2)
  med_s <- as.numeric(med_s)
  # Restore edges (filter returns NA at boundaries)
  med_s[1] <- med[1]; med_s[2] <- med[2]
  med_s[n] <- med[n]; med_s[n-1] <- med[n-1]
  # Fill any remaining interior NAs
  med_s[is.na(med_s)] <- med[is.na(med_s)]
...
for (i in 1:K) {
  ...
  y_med <- smooth_median(boot_proxy$median[, i], y_lo, y_hi)
...
for (i in 1:K) {
  ...
  y_hi  <- boot_proxy$upper[, i]
  y_med <- smooth_median(boot_proxy$median[, i], y_lo, y_hi)
  y_ch  <- smooth_median(boot_chol$median[, i],
                         boot_chol$lower[, i], boot_chol$upper[, i])
