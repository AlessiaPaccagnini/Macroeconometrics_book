...
def smooth_median(med, lo, hi, window=5):
    """Smooth bootstrap median with moving average, clipped to stay inside bands."""
    med_s = uniform_filter1d(med, size=window)
    # Keep first and last 2 points unsmoothed (edge effects)
    med_s[0] = med[0]; med_s[1] = med[1]
    med_s[-1] = med[-1]; med_s[-2] = med[-2]
    # Clip to guarantee line stays inside bands
    med_s = np.clip(med_s, lo, hi)
    return med_s
...
for i, (ax, label) in enumerate(zip(axes, var_labels)):
    med_s = smooth_median(irf_median_proxy[:, i],
                          irf_lower_proxy[:, i], irf_upper_proxy[:, i])
    ax.fill_between(range(H + 1), irf_lower_proxy[:, i], irf_upper_proxy[:, i],
                    alpha=0.25, color=COLORS['proxy'])
    ax.plot(range(H + 1), med_s, color=COLORS['proxy'], linewidth=2.5)
...
for i, (ax, label) in enumerate(zip(axes, var_labels)):
    # Proxy SVAR (smoothed median)
    med_s = smooth_median(irf_median_proxy[:, i],
                          irf_lower_proxy[:, i], irf_upper_proxy[:, i])
    ax.fill_between(range(H + 1), irf_lower_proxy[:, i], irf_upper_proxy[:, i],
                    alpha=0.2, color=COLORS['proxy'])
    ax.plot(range(H + 1), med_s, color=COLORS['proxy'],
            linewidth=2.5, label='Proxy SVAR (R-R)')

    # Cholesky (smoothed median)
    med_c = smooth_median(irf_median_chol[:, i],
                          irf_lower_chol[:, i], irf_upper_chol[:, i])
    ax.plot(range(H + 1), med_c, color=COLORS['cholesky'],
