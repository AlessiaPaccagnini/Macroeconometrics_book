...
for i = 1:K
    subplot(1,3,i);
    med_s = smooth_median(irf_med_proxy(:,i), irf_lo_proxy(:,i), irf_hi_proxy(:,i), 5);
    fill([horizons, fliplr(horizons)], ...
         [irf_lo_proxy(:,i)', fliplr(irf_hi_proxy(:,i)')], ...
         col_proxy, 'FaceAlpha', 0.25, 'EdgeColor', 'none'); hold on;
    plot(horizons, med_s, 'Color', col_proxy, 'LineWidth', 2.5);
...
for i = 1:K
    subplot(1,3,i);
    med_s_p = smooth_median(irf_med_proxy(:,i), irf_lo_proxy(:,i), irf_hi_proxy(:,i), 5);
    med_s_c = smooth_median(irf_med_chol(:,i), irf_lo_chol(:,i), irf_hi_chol(:,i), 5);
    fill([horizons, fliplr(horizons)], ...
         [irf_lo_proxy(:,i)', fliplr(irf_hi_proxy(:,i)')], ...
         col_proxy, 'FaceAlpha', 0.20, 'EdgeColor', 'none'); hold on;
    h1 = plot(horizons, med_s_p, 'Color', col_proxy, 'LineWidth', 2.5);
    h2 = plot(horizons, med_s_c, 'Color', col_chol,  'LineWidth', 2.5, 'LineStyle', '--');
    yline(0, 'k-', 'LineWidth', 0.8);
    xlabel('Quarters after shock');
    ylabel('Percentage points');
    title(['Response of ', var_names{i}]);
...
function med_s = smooth_median(med, lo, hi, window)
% SMOOTH_MEDIAN  Smooth bootstrap median with moving average, clipped inside bands
%   med_s = smooth_median(med, lo, hi, window)
%   med, lo, hi: (H+1) x 1 vectors
%   window: smoothing window (default 5)

    n = length(med);
    kernel = ones(window, 1) / window;
    med_s = conv(med, kernel, 'same');

    % Restore edges (conv 'same' gives edge effects)
    med_s(1) = med(1); med_s(2) = med(2);
    med_s(n) = med(n); med_s(n-1) = med(n-1);

    % Clip to stay inside bands
    med_s = max(med_s, lo);
    med_s = min(med_s, hi);
end
